	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"secondary_ip_count": {
				...
				Optional:     true,
				ForceNew:     true,
				ValidateFunc: validate.ValidateSecondaryIPCount,
				DiffSuppressFunc: func(k, o, n string, d *schema.ResourceData) bool {
...
func getBareMetalOrderFromResourceData(d *schema.ResourceData, meta interface{}) (datatypes.Hardware, error) {
	...
	if public_subnet, ok := d.GetOk("public_subnet"); ok {
		...
		if err != nil {
			return hardware, fmt.Errorf("[ERROR] Error determining id for subnet %s: %s", subnet, err)
		}
...
func resourceIBMComputeBareMetalCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] Encountered problem trying to configure bare metal server options: %s", err)
	}
	...
	if err != nil {
		return fmt.Errorf("[ERROR] Error ordering bare metal server: %s\n%+v\n", err, order)
	}
...
func resourceIBMComputeBareMetalRead(d *schema.ResourceData, meta interface{}) error {
	...
	if len(storages) > 0 {
		d.Set("block_storage_ids", flex.FlattenBlockStorageID(storages))
		d.Set("file_storage_ids", flex.FlattenFileStorageID(storages))
	}
...
func deleteHardware(d dataRetriever, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] Error getting billing item for bare metal server: %s", err)
	}
	...
	if err != nil {
		return fmt.Errorf("[ERROR] Error canceling the bare metal server (%d): %s", id, err)
	}
...
func setHardwareTags(id int, d dataRetriever, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[ERROR] Could  not set tags on bare metal server %d", id)
	}
...
func getItemPriceId(items []datatypes.Product_Item, categoryCode string, keyName string, capacity ...int) (datatypes.Product_Item_Price, error) {
	...
	return datatypes.Product_Item_Price{},
		fmt.Errorf("[ERROR] Could  not find the matching item with categorycode %s and keyName %s. Available item(s) is(are) %s", categoryCode, keyName, availableItems)
}
...
func getMonthlyBareMetalOrder(d *schema.ResourceData, meta interface{}) (datatypes.Container_Product_Order, error) {
	...
	if !ok {
		return datatypes.Container_Product_Order{}, fmt.Errorf("[ERROR] The attribute 'package_key_name' is not defined.")
	}

	datacenter, ok := d.GetOk("datacenter")
	if !ok {
		return datatypes.Container_Product_Order{}, fmt.Errorf("[ERROR] The attribute 'datacenter' is not defined.")
	}

	osKeyName, ok := d.GetOk("os_key_name")
	if !ok {
		return datatypes.Container_Product_Order{}, fmt.Errorf("[ERROR] The attribute 'os_key_name' is not defined.")
	}
...
func setMonthlyHourlyCommonOrder(d *schema.ResourceData, items []datatypes.Product_Item, order *datatypes.Container_Product_Order) error {
	...
	if secondaryIPCount > 0 {
		if privateNetworkOnly {
			return fmt.Errorf("[ERROR] Unable  to configure public secondary addresses with a private_network_only option")
		}
	...
	if d.Get("ipv6_enabled").(bool) {
		if privateNetworkOnly {
			return fmt.Errorf("[ERROR] Unable  to configure a public IPv6 address with a private_network_only option")
		}
...
func setCommonBareMetalOrderOptions(d *schema.ResourceData, meta interface{}, order datatypes.Container_Product_Order) (datatypes.Container_Product_Order, error) {
	...
	if public_subnet, ok := d.GetOk("public_subnet"); ok {
		...
		subnetId, err := getSubnetId(subnet, meta)
		if err != nil {
			return datatypes.Container_Product_Order{}, fmt.Errorf("[ERROR] Error determining id for subnet %s: %s", subnet, err)
		}
...
func findNetworkItemPriceId(items []datatypes.Product_Item, d dataRetriever) (datatypes.Product_Item_Price, error) {
	...
	return datatypes.Product_Item_Price{},
		fmt.Errorf("[ERROR] Could  not find the network with %s, %s, %s, and private_network_only = %t",
...
func getPackageByModel(sess *session.Session, model string) (datatypes.Product_Package, error) {
	...
	return datatypes.Product_Package{}, fmt.Errorf("[ERROR] No custom bare metal package key name for %s. Available package key name(s) is(are) %s", model, availableModels)
}
